...
/** 设置 提示图片 -> 格式: .LeeImage(@@"") */
///!!!:自定义需求
@property (nonatomic, copy, readonly) LEEConfigToImage LeeImage;
...
/** 设置 添加提示图片 -> 格式: .LeeConfigImage(^(UIImageView *imageView){ //code.. }) */
///!!!:自定义需求
@property (nonatomic, copy, readonly) LEEConfigToConfigImage LeeAddImageView;
...
/** 设置 头部内的间距 -> 格式: .LeeHeaderInsets(UIEdgeInsetsMake(20, 0, 18, 0)) */
...
/** 设置 是否显示关闭按钮,默认为NO -> 格式: .LeeShouldShowCloseBtn(NO) */
@property (nonatomic, copy, readonly) LEEConfigToBool LeeShouldShowCloseBtn;

/** 设置 阴影偏移 -> 格式: .LeeShadowOffset(CGSizeMake(0.0f, 0.0f)) */
